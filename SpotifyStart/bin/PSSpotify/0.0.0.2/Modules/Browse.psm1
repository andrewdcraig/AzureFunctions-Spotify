...
function Get-SpotifyFeaturedPlaylists {
    ...
    begin {
        ...
        $Global:SpotifySession = $Session = Assert-AuthToken -TokenEndpoint $Session.TokenEndpoint `
            -ClientIdSecret $Global:SpotifyCredential `
            -RefreshToken $Session.RefreshToken `
            -RootAPIEndpoint $Session.RootUrl `
            -SessionClassName $Session.GetType().FullName `
            -Session $Session
    }
    ...
    process {
        ...
        Invoke-RestMethod -Headers $Session.Headers `
            -Uri $Url `
            -Method Get | select -ExpandProperty playlists | select -ExpandProperty items
    }
}
...
function Get-SpotifyNewReleases {
    ...
    process {
        ...
        Invoke-RestMethod -Headers $Session.Headers `
            -Uri $Url `
            -Method Get | select -ExpandProperty albums | select -ExpandProperty Items
    }
}
...
function Get-SpotifyCategory {
    ...
    process {
        ...
        if ($Categories | gm | select -ExpandProperty Name) {
            $Categories.Items
        }
        else {
            $Categories
...
function Get-SpotifyRecommendation {
    ...
    process {
        ...
        $TotalSeeds = $SeedArtist.count + $SeedTrack.Count + $SeedGenre.count
        ...
        if ($PSBoundParameters.ContainsKey("SeedGenre")) {
            $Query += "seed_genres=$($SeedGenre -join ',')"
        }
...
function Get-SpotifyRecommendationGenres {
    ...
    process {
        ...
        Invoke-RestMethod -Headers $Session.Headers `
            -Uri $Url `
            -Method Get | select -ExpandProperty genres
    }
}